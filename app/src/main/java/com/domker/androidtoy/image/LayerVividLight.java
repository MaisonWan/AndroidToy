 ...
 * TODO
 * @author maisonwan
 * @date 2014-2-21 下午4:52:31
 *
 */
public class LayerVividLight implements LayerBlendable {
	...
	public int pixelCalc(int down, int up) {
		if (down < 128) {
			return down == 0 ? 2 * down : Math.max(0, (255 - ((255 - up) << 8) / (2 * down)));
		} else {
			return (2 * (down - 128)) == 255 ? (2 * (down - 128)) : Math.min(255, ((up << 8 ) / (255 - (2 * (down - 128)))));
		}
	}
